...
for ($sri=0; $sri < count($x_ueraccess_str)-1; $sri++) { 
	...
	$salesrp_arrs .= "'".$x_ueraccess_str[$sri]."'". ",";
}
$salesrp_arrs = rtrim($salesrp_arrs, ",");
// echo($salesrp_arrs);
...
(select DISTINCT costcentre,round(SUM(amount) ,2) as amount FROM crm_salesreport WHERE vdate BETWEEN '$start_date'  and '$end_date' and costcentre IN ($salesrp_arrs)  GROUP BY costcentre)
UNION
(select DISTINCT costcentre,round(SUM(amount) ,2) as amount FROM crm_salesreport_indore WHERE vdate BETWEEN '$start_date'  and '$end_date' and costcentre IN ($salesrp_arrs)  GROUP BY costcentre)
UNION
(select DISTINCT costcentre,round(SUM(amount) ,2) as amount FROM crm_salesreport_lucknow WHERE vdate BETWEEN '$start_date'  and '$end_date' and costcentre IN ($salesrp_arrs)  GROUP BY costcentre)
...
{ ?>
	...
	<?php while ($i < $costcentre_count)
	{
		...
		<button type="button" class="accordion">
			<span style="width: 300px;display: inline-block;color: darkblue;font-weight: bold;"><span><i class="fas fa-user-tie"></i></span> <?php echo $arr_acc_title[$i]["costcentre"] ?></span>
			<span style="width: 150px;display: inline-block;color: darkblue;"><i class="fas fa-rupee-sign"></i> <?php echo $totalsell ?></span>
			<span style="width: 150px;display: inline-block;text-align: center;"><?php 
			echo $i==0?'<span style="color:orange;font-size:16px"><i class="fas fa-star"></i><i class="fas fa-star"></i><i class="fas fa-star"></i></span>': '';
			echo $i==1?'<span style="color:orange;font-size:16px"><i class="fas fa-star"></i><i class="fas fa-star"></i></span>': '';
			echo $i==2?'<span style="color:orange;font-size:16px"><i class="fas fa-star"></i></span>': '';
		?></span>
	</button>
	<div class="panel" style="">
		...
		while ($itr < $total_quarter)
		{
			...
			if ($itr>2) {
				$start_yearsss = $end_yearsss;
				$total_monthss = 0;
				$smonth = sprintf("%02d", ($total_monthss+1));
				$endmonth = sprintf("%02d", $smonth+2);
			}
			else{
				$start_yearsss = $start_yearsss;
			...
			$start_date_a = $start_yearsss. '-' . $smonth . '-01';
			$d = cal_days_in_month(CAL_GREGORIAN, $endmonth, $start_yearsss);
			$end_date_a = $start_yearsss . '-' . $endmonth . '-' . $d;
	...
	$start_date_first_month = $start_yearsss . '-' . $first_month_of_quarter . '-01';
	$day_first_month = cal_days_in_month(CAL_GREGORIAN, $first_month_of_quarter, $start_yearsss);
	$end_date_first_month = $start_yearsss . '-' . $first_month_of_quarter . '-' . $day_first_month;
...
$count_firstmonth = count($arr_first_month);
            // -----------------------------------------------------------
$second_month_of_quarter = sprintf("%02d", ($total_monthss +2));
$start_date_second_month = $start_yearsss . '-' . $second_month_of_quarter . '-01';
$day_second_month = cal_days_in_month(CAL_GREGORIAN, $second_month_of_quarter, $start_yearsss);
$end_date_second_month = $start_yearsss . '-' . $second_month_of_quarter . '-' . $day_second_month;
...
$count_secondmonth = count($arr_second_month);
            // -----------------------------------------------------------
$third_month_of_quarter = sprintf("%02d", ($total_monthss+3));
$start_date_third_month = $start_yearsss . '-' . $third_month_of_quarter . '-01';
$day_third_month = cal_days_in_month(CAL_GREGORIAN, $third_month_of_quarter, $start_yearsss);
$end_date_third_month = $start_yearsss . '-' . $third_month_of_quarter . '-' . $day_third_month;
...
$total_monthss_1 = date("F", mktime(0, 0, 0, $total_monthss +1, 10)) . "<br>";
$total_monthss_2 = date("F", mktime(0, 0, 0, $total_monthss +2, 10)) . "<br>";
$total_monthss_3 = date("F", mktime(0, 0, 0, $total_monthss +3, 10)) . "<br>";
